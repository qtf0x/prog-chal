...
bool isOverlap(const std::pair<std::set<unsigned>, std::set<unsigned>>& input) {
    std::set<unsigned> difference1, difference2;

    std::set_difference(input.first.begin(), input.first.end(),
                        input.second.begin(), input.second.end(),
                        std::inserter(difference1, difference1.begin()));

    std::set_difference(input.second.begin(), input.second.end(),
                        input.first.begin(), input.first.end(),
